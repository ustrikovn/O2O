...
❌ ВЫДУМЫВАТЬ то, чего НЕТ в данных!
❌ Использовать умные слова без оснований ("ответственность", "глубинные проблемы")  
❌ Придумывать мотивации которые сотрудник НЕ озвучивал
...
ЕСЛИ ДАННЫХ МАЛО — не выдумывай, скажи что есть!
...
2. ПСИХОЛОГИЧЕСКОЕ СОСТОЯНИЕ (psychological_state)
   ...
   - ВОССТАНОВЛЕНИЕ: улучшение после отдыха, возврат мотивации
   - ПОЗИТИВНЫЙ СДВИГ: изменение отношения к лучшему
...
5. ПОЗИТИВНЫЕ ИЗМЕНЕНИЯ (ОЧЕНЬ ВАЖНО АНАЛИЗИРОВАТЬ!):
   - Сотруднику стало ЛУЧШЕ → ПОЧЕМУ? Что изменилось?
   - После отпуска/отдыха улучшение → вероятно было ВЫГОРАНИЕ
   - Изменил решение (не уходит) → что повлияло?
   - Улучшение настроения → какой фактор сработал?
   
   ЭТО ЗОЛОТАЯ ИНФОРМАЦИЯ для руководителя! Помоги понять:
   - Какова была РЕАЛЬНАЯ причина проблемы
   - Что нужно делать чтобы ЗАКРЕПИТЬ позитив
   - Какие ВЫВОДЫ сделать на будущее
...

{
  "insights": [
    {
      "type": "behavioral_tactic" | "psychological_state" | "hidden_need" | "relationship_dynamic" | "risk" | "positive_shift",
