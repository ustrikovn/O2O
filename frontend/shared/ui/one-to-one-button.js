/**
 * Компонент кнопки для создания one-to-one встречи
 * Полноширинная горизонтальная кнопка в стиле glass morphism
 */
class OneToOneButton {
    constructor(options = {}) {
        this.options = {
            onClick: options.onClick || (() => console.log('Провести one-to-one')),
            text: options.text || 'Провести one-to-one',
            icon: options.icon || 'zap',
            disabled: options.disabled || false,
            variant: options.variant || 'primary', // 'primary' или 'accent'
            ...options
        };
    ...
    createElement() {
        ...
        const variantClass = this.options.variant === 'accent' ? 'glass-button-accent' : 'glass-button-primary';
        const textColorClass = this.options.variant === 'accent' ? 'text-white' : 'text-primary';
...
window.OneToOneButton = OneToOneButton;
