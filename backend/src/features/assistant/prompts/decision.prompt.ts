...
2. ПОЗИТИВНЫЕ ИЗМЕНЕНИЯ (КРИТИЧЕСКИ ВАЖНО!) — сотруднику стало ЛУЧШЕ:
   - "стало лучше", "чувствую себя лучше" → предложи узнать ПОЧЕМУ
   - После отпуска/отдыха улучшение → возможно было выгорание, надо закрепить
   - Передумал уходить → понять что изменилось
   - Изменение настроения в лучшую сторону → важный момент для руководителя
   Priority: high, intervention_type: proactive_question

3. ИНТЕРЕСНОЕ ПОВЕДЕНИЕ — манипуляция, агрессия, уход в себя
   Priority: high, intervention_type: insight

4. ЕСТЬ ИНТЕРПРЕТАЦИЯ — Analyst дал неочевидный вывод
   Priority: medium, intervention_type: insight

5. ПОЛЕЗНЫЙ ВОПРОС — можно предложить конкретный вопрос
   Priority: medium, intervention_type: proactive_question

6. ВОЗМОЖНОСТЬ — шанс улучшить ситуацию
